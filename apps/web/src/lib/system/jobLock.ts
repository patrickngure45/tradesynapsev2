...
export async function renewJobLock(
...
): Promise<{ renewed: true; held_until: string } | { renewed: false; held_until: string | null }> {
  const key = String(params.key).trim();
  const holderId = String(params.holderId).trim();
  const ttlMs = Math.max(1_000, Math.min(60 * 60_000, Math.trunc(params.ttlMs)));
  const ttlSec = Math.max(1, Math.trunc(ttlMs / 1000));
  ...
  const rows = await sql<{ held_until: string }[]>`
    UPDATE ex_job_lock
    SET held_until = now() + make_interval(secs => ${ttlSec}), updated_at = now()
    WHERE key = ${key}
      AND holder_id = ${holderId}
    RETURNING held_until::text AS held_until
  `;
  ...
  if (rows.length > 0) {
    return { renewed: true, held_until: rows[0]!.held_until };
  }

  const [cur] = await sql<{ held_until: string }[]>`
  ...
  return { renewed: false, held_until: cur?.held_until ?? null };
}
