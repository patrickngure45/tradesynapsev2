...
export async function GET(request: Request) {
  ...
  try {
    const result = await retryOnceOnTransientDbError(async () => {
      ...
      const distAll = await sql<{ rarity: string; count: string }[]>`
        SELECT
      ...
      const dist7d = await sql<{ day: string; rarity: string; count: string }[]>`
        SELECT
    ...
    const dep = responseForDbError("admin_arcade_transparency", e);
    if (dep) return dep;
