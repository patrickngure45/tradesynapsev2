...
import { resolveMutation } from "@/lib/arcade/creation";
...
const MODULE_KEY = "mutation";
const MUTATION_COST_SHARDS = 15;
...
export async function POST(request: Request) {
  ...
  const rateLimitRes = await enforceAccountSecurityRateLimit({
    sql,
    request,
    limiterName: "arcade.mutation.reveal",
    windowMs: 60_000,
    max: 20,
    userId: actingUserId,
  });
  ...
  try {
    const out = await retryOnceOnTransientDbError(async () => {
      return await sql.begin(async (tx) => {
        ...
        const safe = await enforceArcadeSafety(txSql as any, { userId: actingUserId, module: MODULE_KEY, shardSpend: MUTATION_COST_SHARDS });
        ...
        const invRows = await txSql<{ id: string; quantity: number }[]>`
          ...
          FOR UPDATE
        `;
        if (!invRows.length) return { kind: "err" as const, err: apiError("not_found") };
        if (Number(invRows[0]!.quantity ?? 0) < 1) return { kind: "err" as const, err: apiError("insufficient_balance") };
        ...
        if (shardQty < MUTATION_COST_SHARDS) {
          return {
            kind: "err" as const,
            err: apiError("insufficient_balance", { details: { message: `Need ${MUTATION_COST_SHARDS} shards to mutate.` } }),
          };
        }
        ...
        const invRemaining = Number(invRows[0]!.quantity) - 1;
        if (invRemaining === 0) {
          await txSql`
          ...
          await txSql`
            ...
            SET quantity = ${invRemaining}, updated_at = now()
            WHERE id = ${invId}::uuid
          `;
        }
        ...
        const shardRemaining = shardQty - MUTATION_COST_SHARDS;
        if (shardRemaining === 0) {
          await txSql`
            DELETE FROM arcade_inventory
          ...
          await txSql`
            UPDATE arcade_inventory
            SET quantity = ${shardRemaining}, updated_at = now()
            WHERE id = ${shardId}::uuid
          `;
        }
        ...
        await logArcadeConsumption(txSql, {
          ...
          quantity: MUTATION_COST_SHARDS,
          context_type: "mutation_cost",
          context_id: action.id,
          module: MODULE_KEY,
          metadata: { cost_shards: MUTATION_COST_SHARDS },
        });
    ...
    const dep = responseForDbError("arcade_mutation_reveal", e);
    if (dep) return dep;
