...
export async function POST(request: Request) {
  ...
  const item = {
    kind: String(input.item.kind).trim(),
    code: String(input.item.code).trim(),
    rarity: String(input.item.rarity).trim(),
  };

  if (item.kind !== "cosmetic") return apiError("invalid_input", { details: "only_cosmetics_can_mutate" });
  ...
  try {
    const out = await retryOnceOnTransientDbError(async () => {
      return await sql.begin(async (tx) => {
        ...
        // Anti-spam: max 8 commits in the last 60s.
        ...
        `;
        if (Number(lim?.c ?? "0") >= 8) return { kind: "err" as const, err: apiError("rate_limit_exceeded") };
        ...
        const [action] = await txSql<{ id: string; requested_at: string }[]>`
          ...
          VALUES (
            ...
            ${serverCommit},
            ${serverSeedB64},
            ${txSql.json({ item, cost_shards: MUTATION_COST_SHARDS })}
          )
    ...
    const dep = responseForDbError("arcade_mutation_commit", e);
    if (dep) return dep;
