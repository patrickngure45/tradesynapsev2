...
import { generateTieredAiResponse, resolveAiTier } from "@/lib/ai/probabilistic";
  ...
  try {
    const out = await retryOnceOnTransientDbError(async () => {
      return await sql.begin(async (tx) => {
        ...
        const safe = await enforceArcadeSafety(txSql as any, { userId: actingUserId, module: MODULE_KEY, shardSpend: ORACLE_COST_SHARDS });
        ...
        const expectedServerCommit = sha256Hex(
          `${action.server_seed_b64}:${action.client_commit_hash}:${action.module}:${action.profile}:${actingUserId}:prompt=${promptHash}`,
        );
        if (expectedServerCommit !== String(action.server_commit_hash ?? "").toLowerCase()) {
          return { kind: "err" as const, err: apiError("invalid_input", { details: "prompt does not match commit" }) };
        }

        const storedPromptHash = String(action.input_json?.prompt_hash ?? "").trim();
        if (storedPromptHash && storedPromptHash !== promptHash) {
          return { kind: "err" as const, err: apiError("invalid_input", { details: "prompt_hash_mismatch" }) };
        }
        ...
        if (shardQty < ORACLE_COST_SHARDS) {
          return {
            kind: "err" as const,
            err: apiError("insufficient_balance", { details: { message: `Need ${ORACLE_COST_SHARDS} shards to ask the Oracle.` } }),
          };
        }

        const resolvedTier = resolveAiTier({
          actionId: action.id,
          userId: actingUserId,
        ...
        const generated = await generateTieredAiResponse({ prompt, tier: resolvedTier.tier });

        // Spend shards.
        const shardId = shardRows[0]!.id;
        const remaining = shardQty - ORACLE_COST_SHARDS;
        if (remaining === 0) {
        ...
        await logArcadeConsumption(txSql, {
          ...
          code: SHARD_CODE,
          rarity: SHARD_RARITY,
          quantity: ORACLE_COST_SHARDS,
          context_type: MODULE_KEY,
          context_id: action.id,
          module: MODULE_KEY,
          metadata: { cost_shards: ORACLE_COST_SHARDS },
        });

        const responseText = clampText(generated.text, 8000);

        let collectible: { kind: string; code: string; rarity: string; label: string } | null = null;
        if (resolvedTier.tier !== "common") {
          const rarity = resolvedTier.tier;
          const code = `ai_template_${action.id}`;
          collectible = { kind: "insight", code, rarity, label: `AI Template (${rarity})` };
          ...
          await txSql`
            ...
            VALUES (
              ...
              ${txSql.json({
                ...
                prompt_preview: String(action.input_json?.prompt_preview ?? ""),
        ...
        const outcomeJson = {
          module: action.module,
          profile: action.profile,
          cost_shards: ORACLE_COST_SHARDS,
          tier: resolvedTier.tier,
          ...
          audit: {
            ...
            random_hash: resolvedTier.audit.random_hash,
            roll: resolvedTier.audit.roll,
            total: resolvedTier.audit.total,
          },
        };
        ...
        await txSql`
          ...
          SET status = 'resolved',
              ...
              outcome_json = ${txSql.json(outcomeJson as any)}
        ...
        await addArcadeXp(txSql as any, {
          userId: actingUserId,
          deltaXp: 1,
          contextRandomHash: resolvedTier.audit.random_hash,
          source: MODULE_KEY,
        });
    ...
    const dep = responseForDbError("arcade_ai_oracle_reveal", e);
    if (dep) return dep;
