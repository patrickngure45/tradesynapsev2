...
export async function POST(request: Request) {
  ...
  const sql = getSql();
  const claimDateIso = utcDateIso(new Date());
  ...
  try {
      ...
      return await sql.begin(async (tx) => {
        ...
        const [action] = await txSql<{ id: string; requested_at: string }[]>`
          ...
          VALUES (
            ...
            ${serverCommit},
            ${serverSeedB64},
            ${txSql.json({ claim_date: claimDateIso })}
          )
          RETURNING id::text AS id, requested_at
        `;

        // Safety: limit draft frequency to once per day for now.
        await txSql`
    ...
    if (anyErr?.code === "23505") {
      return apiError("rate_limit_exceeded", { details: "Draft already used today" });
    }
