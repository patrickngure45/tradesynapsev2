...
export async function POST(request: Request) {
  ...
  const a = {
    kind: String(input.item_a.kind).trim(),
    code: String(input.item_a.code).trim(),
    rarity: String(input.item_a.rarity).trim(),
  };
  const b = {
    kind: String(input.item_b.kind).trim(),
    code: String(input.item_b.code).trim(),
    rarity: String(input.item_b.rarity).trim(),
  };

  if (a.kind !== "cosmetic" || b.kind !== "cosmetic") return apiError("invalid_input", { details: "only_cosmetics_can_fuse" });
  if (a.code === b.code && a.rarity === b.rarity) return apiError("invalid_input", { details: "must_choose_two_distinct_items" });
  ...
  try {
    const out = await retryOnceOnTransientDbError(async () => {
      return await sql.begin(async (tx) => {
        ...
        // Anti-spam: max 6 commits in the last 60s.
        ...
        `;
        if (Number(lim?.c ?? "0") >= 6) return { kind: "err" as const, err: apiError("rate_limit_exceeded") };
        ...
        const [action] = await txSql<{ id: string; requested_at: string }[]>`
          ...
          VALUES (
            ...
            ${serverCommit},
            ${serverSeedB64},
            ${txSql.json({ item_a: a, item_b: b, cost_shards: FUSION_COST_SHARDS })}
          )
    ...
    const dep = responseForDbError("arcade_fusion_commit", e);
    if (dep) return dep;
