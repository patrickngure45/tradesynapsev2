...
import { resolveMissionReward } from "@/lib/arcade/missions";
...
export async function POST(request: Request) {
  ...
  const rateLimitRes = await enforceAccountSecurityRateLimit({
    sql,
    request,
    limiterName: "arcade.missions.reveal",
    windowMs: 60_000,
    max: 20,
    userId: actingUserId,
  });
  ...
  try {
    const out = await retryOnceOnTransientDbError(async () => {
      return await sql.begin(async (tx) => {
        ...
        const todayIso = String(action.input_json?.today ?? "").trim() || new Date().toISOString().slice(0, 10);
        const missionCode = String(action.input_json?.mission_code ?? "").trim() || "unknown";

        const resolved = resolveMissionReward({
          actionId: action.id,
          userId: actingUserId,
        ...
        await txSql`
          ...
          VALUES (
            ${actingUserId}::uuid,
            ${resolved.reward.kind},
            ${resolved.reward.code},
            ${resolved.reward.rarity},
            ${resolved.reward.quantity},
            ${txSql.json({ label: resolved.reward.label, source: MODULE_KEY, mission_code: missionCode, today: todayIso, action_id: action.id })},
            now(),
            now()
          )
    ...
    const dep = responseForDbError("arcade_missions_reveal", e);
    if (dep) return dep;
