...
export async function POST(
  ...
  const rateLimitRes = await enforceAccountSecurityRateLimit({
    sql,
    request,
    limiterName: "trades.status.released",
    windowMs: 60_000,
    max: 20,
    userId: actingUserId,
  });
  ...
  try {
    ...
    const resp = responseForDbError("trades.status.released", e);
    if (resp) return resp;
    throw e;
  }
}
