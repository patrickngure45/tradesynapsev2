...
export async function POST(
  ...
  const rateLimitRes = await enforceAccountSecurityRateLimit({
    sql,
    request,
    limiterName: "trades.status.paid_marked",
    windowMs: 60_000,
    max: 20,
    userId: actingUserId,
  });
  ...
  try {
    ...
    const updated = await txSql<{ id: string }[]>`
      UPDATE trade
      SET status = ${to}, paid_marked_at = now()
    ...
    const resp = responseForDbError("trades.status.paid_marked", e);
    if (resp) return resp;
    throw e;
  }
}
