...
function getRpcUrlForLog(chain: "bsc" | "eth"): string {
  if (chain === "eth") return process.env.ETH_RPC_URL || process.env.ETHEREUM_RPC_URL || "https://cloudflare-eth.com";
...
function getReorgSafetyWindowBlocks(): number {
  // How far back we verify already-recorded events still exist on-chain.
  // Larger = safer but more RPC load.
  const raw = Number.parseInt(process.env.DEPOSIT_REORG_WINDOW_BLOCKS ?? "24", 10);
  if (!Number.isFinite(raw) || raw < 0) return 24;
  return Math.min(500, raw);
}

function isPendingCreditEnabled(): boolean {
